...
public class SellInventory {
    ...
    public SellInventory(Player p) {
        ...
        //Player a = new FarmObject(Metodos.getPlayer(p));
        FarmObject player = Metodos.getPlayer(p.getName());
        int limite = player.getLimite();
        int fungo = player.getFungo();
        int cana = player.getSugar();
        int melancia = player.getMelon();
        inv.setItem(12, (new ItemBuilder(Material.MELON_BLOCK)).setName("§eMelancia!").setLore(new String[] {"§fQuantia: §7"+melancia,"§fLimite de Venda: §7"+limite,"","§aClique para vender!"}).setAmount(1).toItemStack());
        inv.setItem(13, (new ItemBuilder(Material.SUGAR_CANE)).setName("§aCana!").setLore(new String[] {"§fQuantia: §7"+cana,"§fLimite de Venda: §7"+limite,"","§aClique para vender!"}).setAmount(1).toItemStack());
        inv.setItem(14, (new ItemBuilder(Material.getMaterial(372))).setName("§cFungo!").setLore(new String[] {"§fQuantia: §7"+fungo,"§fLimite de Venda: §7"+limite,"","§aClique para vender!"}).setAmount(1).toItemStack());
